 ...
 */
    ...
    override fun onHandleIntent(intent: Intent?) {
        var errorMessage = ""
        mReceiver =
            intent!!.getParcelableExtra(AppUtils.LocationConstants.RECEIVER)
        ...
        val location =
            intent.getParcelableExtra<Location>(AppUtils.LocationConstants.LOCATION_DATA_EXTRA)
        ...
        // Errors could still arise from using the Geocoder (for satmatgroup, if there is no
        ...
        try {
            ...
            addresses = geocoder.getFromLocation(
                location.latitude,
                location.longitude,  // In this sample, we get just a single address.
                1
            )
        ...
        if (addresses == null || addresses.size == 0) {
            ...
            deliverResultToReceiver(
                AppUtils.LocationConstants.SUCCESS_RESULT,
    ...
    private fun deliverResultToReceiver(
        ...
        try {
            ...
            bundle.putString(
                AppUtils.LocationConstants.LOCATION_DATA_AREA,
                address!!.subLocality
            )
            bundle.putString(AppUtils.LocationConstants.LOCATION_DATA_CITY, address.locality)
            bundle.putString(
                AppUtils.LocationConstants.LOCATION_DATA_STREET,
